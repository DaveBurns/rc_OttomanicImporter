...
function App:callingTypeAssert( var, name, expType, m, ... )
    if var == nil then
        error( str:fmtx( "'^1' is nil, it was expected have a '^2' value^3", name or "noname", expType or "notype", str:is( m ) and " - "..str:fmtx( m, ... ) or "." ), 3 )
    elseif type( var ) == expType then
        return var
    else
...
function App:parseToolkitId( pluginId )
    local tkId = self:getInfo( "LrToolkitIdentifier" ) or "nil" -- same as _PLUGIN.id, no?
    app:assert( tkId == _PLUGIN.id, "###1" )
    pluginId = pluginId or _PLUGIN.id
